...
### 作業開始時・autocompact後の絶対ルール
**毎回必ず以下のファイルを最初に読み込む:**
1. **📋 project_charter.md** - プロジェクト全体情報の信頼できる情報源
2. **📊 docs/PROJECT_MANAGEMENT.md** - 現在の優先度・TODO/ISSUE一元管理
3. **📊 docs/PROBLEM_LOG.csv** - 過去の問題・教訓集

### ⚠️ Claude Code重大欠陥対策
- **autocompact発生時**: 上記3ファイルを必ず自動再読み込み
- **情報忘却防止**: コンテキスト消失時の自動復旧プロトコル
- **一貫性保持**: 同じミスを繰り返さないための記憶維持
...
### 🔗 環境詳細情報
**詳細な技術スタック・環境構成は `project_charter.md` を参照**
...
4. ❌ **単発対応**: 関連問題の予見・分析回避
5. ❌ **情報資産化未完了**: エラー解決後に知見記録を省略してセッション終了
...
#### 🔒 **Step 6: 情報資産化完了（セッション終了前必須）**
- **絶対ルール**: エラー解決後、知見の完全記録が完了するまでセッション終了禁止
- PROBLEM_LOG.csv への新規事象記録
- project_charter.md への教訓追加（必要に応じて）
- 関連ドキュメントへの知見反映
- 予防策の具体的手順化
- **確認完了まで絶対にセッションを終わらせない**
...
## 📋 現在の開発状況
**詳細な開発状況・TODO/ISSUEは `docs/PROJECT_MANAGEMENT.md` を参照**
...
### 🔗 プロジェクト管理文書
**詳細な管理情報は以下を参照:**
- **📋 project_charter.md** - プロジェクト憲章（総合情報源）
- **📋 docs/PROJECT_MANAGEMENT.md** - TODO/ISSUE管理
- **📊 docs/PROBLEM_LOG.csv** - 問題・教訓記録
- **📝 docs/MIGRATION_LOG.md** - 移行記録
- **📊 docs/CI_CD/監視システム運用ガイド.md** - 監視ガイド
...
### 毎セッション開始時の必須チェック
1. **📋 project_charter.md読み込み** - プロジェクト全体情報の確認
2. **このCLAUDE.mdを確認** - 特に🚨眼の前のエラー対応厳格ワークフロー
3. **📊 docs/PROBLEM_LOG.csv確認** - 過去の問題・教訓の復習必須
4. **📋 docs/PROJECT_MANAGEMENT.md確認** - 今日の最高/高優先度項目
5. **未解決ISSUE確認** - バグ(B)/プロセス(P)の優先対応項目  
6. **小さなエラー・警告チェック**: 見落としがちな問題の洗い出し
7. **SSH接続テスト**: `ssh conoha-vps "echo 'SSH接続成功'"`  
8. **アプリケーション状態確認**: `curl -I https://uchinokiroku.com/`
9. **VPSビルド状態確認**: `.next`ディレクトリの最新性確認

### 🚨 autocompact後の必須プロトコル
**autocompact・コンテキスト消失検出時は必ず以下を実行:**
```
1. project_charter.md の読み込み
2. docs/PROJECT_MANAGEMENT.md の読み込み
3. docs/PROBLEM_LOG.csv の読み込み
4. 現在の作業状況の復旧確認
```
**このプロトコルを怠ると過去の教訓を忘れ、同じミスを繰り返す重大リスクあり**
...
9. **🔒 情報資産化完了確認** - エラー・問題対応時は知見記録まで必須
10. 次回作業メモの更新
...
**🚨 新体制**: project_charter.md統合・autocompact対策プロトコル確立（2025年8月16日）  
**🔒 最重要**: 情報資産化完了までセッション終了禁止（2025年8月16日P007重大反省）**