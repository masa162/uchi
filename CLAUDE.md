...
### 毎セッション開始時の必須チェック
1. **このCLAUDE.mdを最初に確認** - 特に🚨エラー・後回し管理ワークフロー
2. **📋 PROJECT_MANAGEMENT.md確認** - 今日の最高/高優先度項目
3. **未解決ISSUE確認** - バグ(B)/プロセス(P)の優先対応項目
4. **SSH接続テスト**: `ssh conoha-vps "echo 'SSH接続成功'"`  
5. **アプリケーション状態確認**: `curl -I https://uchinokiroku.com/`

### 作業終了時の必須事項
1. **エラー・警告のISSUE記録** - 未解決項目は必ずPROJECT_MANAGEMENT.mdに登録
2. 変更内容のコミット・プッシュ
3. VPS状態の確認
